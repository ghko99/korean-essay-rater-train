...
```bash
git clone https://github.com/ghko99/korean-essay-rater-train.git
cd korean-essay-rater-train
pip install -r requirements.txt
```

> **Note**: Git LFS가 설치되어 있어야 `data_precomputed/` 내 학습 데이터가 정상적으로 다운로드됩니다.
> ```bash
> # Git LFS 미설치 시
...
```bash
# run_train.sh 내 BASE_MODEL 경로를 EXAONE 모델 위치에 맞게 수정 후 실행
bash run_train.sh
